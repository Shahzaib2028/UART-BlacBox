...
#endif
#include <iostream>
class UART_CORE_api_t: public sim_api_t<VerilatorDataWrapper*> {
    public:
    UART_CORE_api_t(VUART_CORE* _dut) {
        dut = _dut;
        main_time = 0L;
        is_exit = false;
#if VM_TRACE
    ...
    void init_sim_data() {
        ...
        sim_data.inputs.push_back(new VerilatorCData(&(dut->io_rx_i)));
        ...
        sim_data.outputs.push_back(new VerilatorCData(&(dut->io_intr_tx)));
        sim_data.outputs.push_back(new VerilatorCData(&(dut->io_tx_o)));
        sim_data.outputs.push_back(new VerilatorIData(&(dut->io_rdata)));
        sim_data.signals.push_back(new VerilatorCData(&(dut->reset)));
        sim_data.signal_map["UART_CORE.reset"] = 0;
    }
#if VM_TRACE
...
static UART_CORE_api_t * _Top_api;
...
int main(int argc, char **argv, char **env) {
    ...
    std::string vcdfile = "test_run_dir/UART_TEST/UART_CORE.vcd";
    ...
    Verilated::mkdir("test_run_dir/UART_TEST/logs");
    VerilatedCov::write("test_run_dir/UART_TEST/logs/coverage.dat");
#endif
    delete top;
    exit(0);
}
